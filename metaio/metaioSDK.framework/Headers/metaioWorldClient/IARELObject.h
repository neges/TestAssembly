...
struct ObjectButton
{
	std::string buttonID;		//!< ID of the button
	std::string buttonName;		//!< Name of the button
	std::string buttonValue;	//!< Value of the button
...
struct ObjectPopup
{
	...
	std::vector<ObjectButton>   buttons;			//!< All the buttons
...
public:
	 ...
	 */
	virtual stlcompat::String getLocalFileForURL(const stlcompat::String& url) const = 0;
	...
	/** returns true if the POI is an advertisement poi */
	virtual bool isAdPOI() const = 0;
	 ...
	 * */
	virtual stlcompat::String getARELParameter(const std::string& parameterName) const = 0;
